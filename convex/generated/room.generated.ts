...
function roomCode(state: number): { code: string; state: number } {
    ...
    let rngState = state
    let code = ''
    for (const unused of [0, 1, 2, 3]) {
        void unused
        const draw = drawBounded(rngState, alphabet.length)
        rngState = draw.state
        code = code + (alphabet.at(draw.value) ?? alphabet[0])
    }
    return { code: code, state: rngState }
}

export async function createRoom(
    ctx: MutationCtx,
    hostAuthId: string | undefined,
    seed: number | undefined,
): Promise<string> {
    let rngState = normalizeSeed(seed ?? Date.now())
...
export async function joinRoom(
    ...
    if (namedPlayer) {
        ...
        if (authId && !namedPlayer.authId) {
            await ctx.db.patch(namedPlayer._id, { authId: authId })
        }
...
export async function startRoom(ctx: MutationCtx, roomId: Id<'rooms'>): Promise<void | null> {
    ...
    const players = await ctx.db
        ...
        .take(4)
    if (!players.length) {
        throw new ConvexError('At least one hero must join.')
    }
...
async function clearSelection(ctx: MutationCtx, roomId: Id<'rooms'>): Promise<void> {
...
export async function rollMovement(
    ...
    for (const sides of player.dice) {
        const draw = drawBounded(rngState, sides)
        rngState = draw.state
        results.push(draw.value + 1)
    }
...
export async function selectDestination(
    ctx: MutationCtx,
    args: { roomId: Id<'rooms'>; playerId: Id<'players'>; destination: number },
): Promise<void> {
    ...
    const path = current
        ...
        : []
    if (!path) {
        throw new ConvexError('That road cannot be used from here.')
    }
    ...
    if (current) {
        await ctx.db.replace(current._id, value)
    } else {
        await ctx.db.insert('roomSelections', value)
    }
    const nodeLabel = getNode(previewDestination).label
    const message = remaining
        ? 'Planning through ' + nodeLabel + '. ' + (remaining + ' movement left.')
        : 'Route ends at ' + nodeLabel + '. Press A to travel.'
    await ctx.db.patch(roomId, { message: message })
}
